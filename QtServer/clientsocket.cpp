...
vector<bool> ClientSocket::PortID(MAXCONN,false);
...
void ClientSocket::onReadyRead()
{
    ...
    while(socket->read((char*)&user, sizeof(user))> 0){
        switch(user.type){
            ...
            break;
            case AUDIO:
                server_chatText (user);     //开启语音
                break;
            case UNAUDIO:
                server_chatText (user);     //关闭语音
                break;
            default:break;
        }
    }
...
void ClientSocket::server_exit(user_t user)
{
    ...
    mutex2.lock();
    int portid = od->getportidOnline(pname);
    od->delOnline(pname);
    PortID[portid] = false;
    qDebug() << portid;
    user.portId = 0;
    qDebug() <<PortID[0]<<PortID[1]<<PortID[2]<<PortID[3];
    mutex2.unlock();
...
//刚进入聊天室时获取聊天室的portID
void ClientSocket::server_portID(user_t user){
    qDebug()<< __FUNCTION__ <<"\t\tin";
    showUser_t(user);
    ...
    OnlineDao *od = new OnlineDaoImp();
    mutex2.lock();
    int portid = od->getportidOnline(roomName);
    mutex2.unlock();
    user.portId = portid;
...
void ClientSocket::server_chatText (user_t user)
{
    ...
    for(it = rooms.begin (); it != rooms.end (); ++it){
        ...
        if(chat.getRoomName () == roomName){
            //qDebug()<< chat.getRoomName() << " " << chat.getUserName();
...
//刷新聊天室列表
 void ClientSocket::server_roomList(user_t user)
 {
     ...
     for(it = v_name.begin (); it != v_name.end (); ++it){
         QString q_name = *it;
         mutex2.lock();
         int portid = od->getportidOnline(q_name);
         qDebug()<< "name="<<q_name<<" "<<"portid="<<QString::number(portid);
         mutex2.unlock();
